...
class SourceDynamoDBSinger(SingerSource):
    TAP_CMD = "tap-dynamodb"

    def check_config(
        self, logger: AirbyteLogger, config_path: str, config: json
    ) -> AirbyteConnectionStatus:
        """
        Tests if the input configuration can be used to successfully connect to the integration
            e.g: if a provided API token can be used to connect to the API.
        :param logger: Logging object to display debug/info/error to the logs
            (logs will not be accessible via UI if they are not passed to this logger)
